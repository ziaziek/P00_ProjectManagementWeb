...
alter table users add activation_code text;
...
returns void as
'insert into users(email, role, pass, active, activation_code) values($1, $2, $3, false, $4);
insert into users_activation(user_id, activation_code) values((select currval(''users_id_seq'')), now()+($5 * interval ''1 minute''));'
...
returns integer as
'insert into userloginhistory(user_id, ip_address, client_name) values((select id from users where email=$1 limit 1), $2, $3);
